...
ticketsRouter.get('/:id', async (req, res) => {
    ...
    if (ticket.totalSlaHours) {
        ...
        const slaStartStage = ticket.stages.find(stage => stage.key === 'CONFIRM_DUE' ||
...
ticketsRouter.post('/:id/transition', async (req, res) => {
    const { id } = req.params;
    ...
    if (!nexts.includes(to))
        return res.status(400).json({ error: `Invalid transition from ${existing.status} to ${to}` });
    const now = new Date();
    ...
    const updateData = {
        ...
        ...((computedSla ?? null) || typeof slaHours === 'number'
            ? { totalSlaHours: (computedSla ?? slaHours) }
            : {}),
    };
...
ticketsRouter.get('/sla-performance', async (_req, res) => {
    try {
...
ticketsRouter.get('/:id/mttr', async (req, res) => {
    const { id } = req.params;
    try {
